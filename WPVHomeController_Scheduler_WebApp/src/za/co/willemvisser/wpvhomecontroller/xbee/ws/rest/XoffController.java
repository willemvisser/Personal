...
@Path("/xoff")
public class XoffController {
	
	static Logger log = Logger.getLogger(XoffController.class.getName());
	...
	//WV QUeryString Example:  http://192.168.1.202:8080/WPVHomeController_Scheduler_WebApp/xoff/1/D/7
    ...
    public String showMsg(@PathParam("device") String device, @PathParam("param1") String param1, @PathParam("param2") String param2){  
		...
		log.info("xoff command:  " + device + " - param1: " + param1 + " - param2: " + param2);


		try {
			RemoteAtResponse response = XbeeController.INSTANCE.remoteAtRequest(device, param1 + param2, new int[] {4});
